...
const EquipmentForm = () => {
  ...
  const mutation = useMutation({
    mutationFn: (values: FormData) => 
      id ? equipmentService.update(Number(id), values) : equipmentService.create(values),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['equipment'] });
      enqueueSnackbar(
        `Equipment ${id ? 'updated' : 'created'} successfully`,
        { variant: 'success' }
      );
      navigate('/equipment');
    },
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (manualFile) formData.append('manual_file', manualFile);
